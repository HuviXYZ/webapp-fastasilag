...
app.post("/registrerlag", (req, res) => {
    ...
    db.prepare("INSERT INTO fantasilag (lagnavn, lageier, S, VM, M, HM, VB, K) VALUES (?,?,?,?,?,?)").run(svar.lagnavn, req.session.brukerid, svar.spiss, svar.venstremidt, svar.midt, svar.hoyremidt, svar.venstreback, svar.keeper)
    res.redirect("/")}
    else {res.redirect("back")}
})
